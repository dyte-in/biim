...
class SectionParser:
    ...
    def push(self, packet):
        begin = ts.HEADER_SIZE + \
            (1 + ts.adaptation_field_length(packet)
             if ts.has_adaptation_field(packet) else 0)
        ...
        if ts.payload_unit_start_indicator(packet):
            while begin < ts.PACKET_SIZE:
                if packet[begin] == ts.STUFFING_BYTE[0]:
                    break
                if self.section:
                    next = min(begin + self.section.remains(packet),
                               ts.PACKET_SIZE)
                else:
                    section_length = (
                        (packet[begin + 1] & 0x0F) << 8) | packet[begin + 2]
                    next = min(begin + (3 + section_length), ts.PACKET_SIZE)
                    self.section = bytearray()
                self.section += packet[begin:next]

                section_length = (
                    (self.section[1] & 0x0F) << 8) | self.section[2]
...
class PESParser:
    ...
    def push(self, packet):
        begin = ts.HEADER_SIZE + \
            (1 + ts.adaptation_field_length(packet)
             if ts.has_adaptation_field(packet) else 0)
        if not ts.payload_unit_start_indicator(packet) and not self.pes:
            return
        ...
        if ts.payload_unit_start_indicator(packet):
            ...
            if pes_length == 0:
                ...
                next = min(begin + (PES.HEADER_SIZE +
                           pes_length), ts.PACKET_SIZE)
            ...
            if pes_length == 0:
                ...
                next = min(begin + (len(self.pes) -
                           (PES.HEADER_SIZE + pes_length)), ts.PACKET_SIZE)
